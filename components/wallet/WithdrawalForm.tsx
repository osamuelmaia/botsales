      ...
      <form onSubmit={handleSubmit(onSubmit)} className="p-5 space-y-4">
        ...
        <div>
          <label className={labelCls}>
            Valor (disponível: {formatBRL(balanceCents)})
          </label>
          ...
          {errors.amountCents && (
            <p className="text-xs text-red-500 mt-1">{errors.amountCents.message}</p>
          )}
        </div>
        ...
        <button
          ...
          className="w-full h-9 rounded-lg bg-gray-900 text-sm text-white font-medium hover:bg-gray-800 disabled:opacity-50 transition-colors"
        >
        ...
        {balanceCents <= 0 && (
          <p className="text-xs text-center text-gray-400">
            Saldo disponível insuficiente para saque
          </p>
        )}
      </form>
    </div>
