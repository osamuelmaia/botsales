  ...
  return (
    <Popover.Root open={open} onOpenChange={setOpen}>
      ...
      <Popover.Portal>
        ...
        >
          {/* ── Presets — vertically centered so they align with the calendar ── */}
          <div className="w-[120px] border-r border-gray-100 flex flex-col justify-center py-4 shrink-0">
          ...
          <div className="p-4 w-[268px]">
            ...
            <div className="grid grid-cols-7 mb-1">
              {["D","S","T","Q","Q","S","S"].map((d, i) => (
                <div key={i} className="text-[10px] font-semibold text-gray-300 text-center py-0.5">
                  {d}
                </div>
              ))}
            </div>
              ...
              {days.map((day, i) => {
                ...
                return (
                  <div key={i} className="relative h-8">
                    ...
                    {inRange   && <div className="absolute inset-y-1.5 inset-x-0 bg-gray-100" />}
                    {bandRight && <div className="absolute inset-y-1.5 left-1/2 right-0 bg-gray-100" />}
                    {bandLeft  && <div className="absolute inset-y-1.5 left-0 right-1/2 bg-gray-100" />}
                    ...
                    <button
                      ...
                      className={[
                        ...
                        inMonth && !isEdge && !inRange && "text-gray-700 hover:bg-gray-100 hover:text-gray-900 cursor-pointer",
                        inRange  && !isEdge && "text-gray-700 font-medium",
                        isEdge   && inMonth && "bg-gray-900 text-white font-bold",
                        isToday  && !isEdge && inMonth && "font-bold text-gray-900",
                      ].filter(Boolean).join(" ")}
                    >
                      {isToday && !isEdge && inMonth
                        ? <span className="relative after:absolute after:bottom-0.5 after:left-1/2 after:-translate-x-1/2 after:w-1 after:h-1 after:rounded-full after:bg-gray-400">{format(day, "d")}</span>
                        : format(day, "d")
                      }
                    </button>
                  </div>
            ...
            <div className="mt-3 pt-3 border-t border-gray-100 flex items-center justify-center gap-3 text-xs">
              <span className={draft.from ? "text-gray-700 font-medium" : "text-gray-300"}>
                {draft.from ? format(draft.from, "dd MMM yyyy", { locale: ptBR }) : "data inicial"}
              </span>
              <span className="text-gray-300">→</span>
              <span className={rangeTo ? "text-gray-700 font-medium" : "text-gray-300"}>
                {rangeTo ? format(rangeTo, "dd MMM yyyy", { locale: ptBR }) : "data final"}
              </span>
            </div>
          </div>
        </Popover.Content>
